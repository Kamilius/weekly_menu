        ...
        return $http.get("/api/calendar/" + encodeURIComponent(formatDateString($scope.weeklyMenu[0].date))).success(function(data) {
            return data.forEach(function(dataDay) {
                ...
                if (dayOfWeek) {
                    ...
                    return dayOfWeek[dataDay.meal].push(new Recipe(recipe.id, recipe.name, recipe.description, dataDay.meal, recipe.ingredients));
                }
            });
        });
    };
    ...
    setFormModel = function() {
        if ($routeParams.recipeId) {
            return $http.get("/api/recipes/" + $routeParams.recipeId).success(function(data, status, headers, config) {
                if (data) {
                    return $scope.recipe = new Recipe(data.id, data.name, data.description, "", data.ingredients);
                }
            });
        } else {
...
app.controller("TopNavCtrl", [ "$scope", "$http", function($scope, $http) {
    return $scope.signOut = function() {
        return $http.get("/api/signout").success(function(data) {
            if (data.message === "success") {
                return $scope.$root.userAuthenticated = false;
            }
        }).error(function() {
...
Recipe = function() {
    ...
    function Recipe(id, name, description, meal, ingredients) {
        this.id = id != null ? id : 0;
        this.name = name != null ? name : "";
    ...
    setRecipes = function(data) {
        if (data) {
            return recipes = data.map(function(recipe) {
                return new Recipe(recipe.id, recipe.name, recipe.description, recipe.meal, recipe.ingredients.map(function(ing) {
...
app.run([ "$rootScope", "$location", "$http", function($rootScope, $location, $http) {
    ...
    $http.get("/api/authentication").success(function(data) {
        return $rootScope.userAuthenticated = data.authenticated;
    });
