...
app.controller("CalendarCtrl", [ "$scope", "$http", "$filter", "recipeService", function($scope, $http, $filter, $recipeService) {
    ...
    getDayByDate = function(date) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            day = _ref[_i];
            if (day.date.toLocaleDateString() === date) {
                return day;
            }
        }
    };
    ...
    $scope.removeRecipe = function(recipe, day) {
        ...
        return $http["delete"](deleteURL).success(function(data, status, headers, config) {
            ...
            if (data.message === "error") {
                return $scope.$root.setStatusMessage("Виникла помилка. Спробуйте видалити рецепт іще раз.", "error");
            } else {
                day[recipe.meal].splice(day[recipe.meal].indexOf(recipe), 1);
                return $scope.$root.setStatusMessage("Рецепт успішно видалено", "success");
            }
        });
    };
    ...
    getRecipesForCurrentWeek = function() {
        return $http.get("/api/calendar/" + encodeURIComponent($scope.weeklyMenu[0].date.toLocaleDateString())).success(function(data) {
            ...
            if (droppable.classList.contains("meal")) {
                ...
                if (!recipeInMeal(dayName, mealName, draggableId)) {
                    $http.post("/api/calendar", {
                        ...
                        if (recipe) {
                            return day[mealName].push(recipe);
                        }
                    });
                } else {
