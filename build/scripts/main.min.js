...
(function(p, e, B) {
    ...
    function z(e, h, f) {
        return {
            ...
            link: function(a, b) {
                ...
                b.html(g.$template);
                var x = e(b.contents());
                c.controller && (g.$scope = a, g = h(c.controller, g), c.controllerAs && (a[c.controllerAs] = g), 
                b.data("$ngControllerController", g), b.children().data("$ngControllerController", g));
                x(a);
            }
        };
    }
    p = e.module("ngRoute", [ "ng" ]).provider("$route", function() {
        ...
        this.$get = [ "$rootScope", "$location", "$routeParams", "$q", "$injector", "$templateRequest", "$sce", function(a, b, c, g, h, p, w) {
            ...
            function m() {
                var t = r.current, d = n;
                if (u) t.params = d.params, e.copy(t.params, c), a.$broadcast("$routeUpdate", t); else if (d || t) v = !1, 
                (r.current = d) && d.redirectTo && (e.isString(d.redirectTo) ? b.path(s(d.redirectTo, d.params)).search(d.params).replace() : b.url(d.redirectTo(d.pathParams, b.path(), b.search())).replace()), 
                g.when(d).then(function() {
                    if (d) {
                        ...
                        e.isDefined(b = d.template) ? e.isFunction(b) && (b = b(d.params)) : e.isDefined(c = d.templateUrl) && (e.isFunction(c) && (c = c(d.params)), 
                        c = w.getTrustedResourceUrl(c), e.isDefined(c) && (d.loadedTemplateUrl = c, b = p(c)));
...
(function(M, f, S) {
    ...
    f.module("ngAnimate", [ "ng" ]).directive("ngAnimateChildren", function() {
        ...
        T.decorator("$animate", [ "$delegate", "$$q", "$injector", "$sniffer", "$rootElement", "$$asyncCallback", "$rootScope", "$document", "$templateRequest", function(O, M, I, U, x, C, P, S, V) {
            ...
            function K(a) {
                if (a = k(a)) a = f.isFunction(a.getElementsByClassName) ? a.getElementsByClassName("ng-animate") : a.querySelectorAll(".ng-animate"), 
                g(a, function(a) {
                    a = f.element(a);
            ...
            function J(a, d, b, c, e) {
                ...
                if (-1 != m.getAttribute("class").indexOf(b) && a) {
                    ...
                    if (0 === x) d.removeClass(q), K(d, b), c(); else {
                        ...
                        0 < p.length && (v = m.getAttribute("style") || "", ";" !== v.charAt(v.length - 1) && (v += ";"), 
...
var formatDateString, getDateOfISOWeek, getMonthNameByNumber;

formatDateString = function(date) {
    var twoSymbolsDate;
    twoSymbolsDate = date.getDate();
    if (twoSymbolsDate.toString().length === 1) {
        twoSymbolsDate = "0" + twoSymbolsDate;
    }
...
getMonthNameByNumber = function(monthNum) {
    switch (monthNum) {
      case 0:
        return "січень";
...
app.controller("AccountCtrl", [ "$scope", "$http", "$location", function($scope, $http, $location) {
    $scope.user = {
        username: "",
        password: "",
        passwordCheck: "",
    ...
    $scope.showSignUpForm = function() {
        return $scope.signUpFormVisible = true;
    };
        ...
        if ($scope.user.username && $scope.user.password) {
            return $http.post("/api/login", {
                ...
                if (data.message === "success") {
                    ...
                    return $scope.$root.setStatusMessage(data.message, "error");
                }
            });
        } else {
    ...
    return $scope.signUp = function() {
        if ($scope.user.username && $scope.user.password === $scope.user.passwordCheck) {
            ...
            return $scope.$root.setStatusMessage("Усі поля є обов'язковими", "error");
        }
    };
} ]);

app.controller("CalendarCtrl", [ "$scope", "$http", "$filter", "recipeService", function($scope, $http, $filter, $recipeService) {
    var buildWeek, currentWeek, currentYear, dayNames, getDayByDate, getDayByName, getRecipesForCurrentWeek, init, recipeInMeal, removeAllRecipesById, todayDate;
    ...
    $scope.showRecipeDetails = function(recipe) {
        return $scope.$root.$broadcast("show-recipe-details", recipe);
    };
    ...
    buildWeek = function() {
        ...
        for (index = _i = 0; _i <= 6; index = ++_i) {
            _results.push($scope.weeklyMenu.push(new DayOfWeek(dayNames[index], new Date(new Date(monday).setDate(monday.getDate() + index)))));
        }
        return _results;
    };
    getDayByName = function(dayName) {
        var day, _i, _len, _ref;
        _ref = $scope.weeklyMenu;
    ...
    getDayByDate = function(date) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            day = _ref[_i];
            if (formatDateString(day.date) === date) {
                return day;
            }
        }
    };
    $scope.nextWeek = function() {
        ...
        buildWeek();
        return getRecipesForCurrentWeek();
    };
    ...
    $scope.removeRecipe = function(recipe, day) {
        ...
        return $http["delete"](deleteURL).success(function(data, status, headers, config) {
            ...
            return day.mealInProgress = "";
        });
    };
    removeAllRecipesById = function(id) {
        var filterHelper;
        filterHelper = function(recipe) {
            return recipe.id !== id;
        };
        return $scope.weeklyMenu.forEach(function(day) {
            day.breakfast = day.breakfast.filter(filterHelper);
            day.lunch = day.lunch.filter(filterHelper);
            return day.dinner = day.dinner.filter(filterHelper);
        });
    };
    recipeInMeal = function(day, meal, recipeId) {
        var recipe, weekDay, _i, _j, _len, _len1, _ref, _ref1;
        _ref = $scope.weeklyMenu;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            weekDay = _ref[_i];
            if (weekDay.name === day) {
    ...
    getRecipesForCurrentWeek = function() {
        return $http.get("/api/calendar/" + encodeURIComponent(formatDateString($scope.weeklyMenu[0].date))).success(function(data) {
            return data.forEach(function(dataDay) {
                var date, dayOfWeek, recipe;
                date = formatDateString(new Date(dataDay.date));
                dayOfWeek = getDayByDate(date);
                if (dayOfWeek) {
                    recipe = dataDay.recipe;
                    return dayOfWeek[dataDay.meal].push(new Recipe(recipe.id, recipe.name, recipe.description, dataDay.meal, recipe.image, recipe.ingredients));
                }
            });
        });
    };
    init = function() {
        ...
        return calendar.addEventListener("drop", function(event) {
            var date, day, dayName, draggableId, droppable, mealName;
            ...
            if (droppable.classList.contains("meal")) {
                ...
                date = day.date;
                if (!recipeInMeal(dayName, mealName, draggableId)) {
    ...
    $scope.$root.$on("recipe:removed", function(event, data) {
        return removeAllRecipesById(data.recipeId);
    });
    return init();
} ]);

app.controller("DetailsOverlayCtrl", [ "$scope", function($scope) {
    $scope.recipeDetails = {
        name: "",
        ingredients: [],
        description: ""
    ...
    $scope.$on("show-recipe-details", function(event, data) {
        $scope.recipeDetails = data;
        return $scope.recipeDetailsVisible = true;
    });
    return $scope.hideRecipeDetails = function() {
        return $scope.recipeDetailsVisible = false;
    };
} ]);

app.controller("IngredientsCtrl", [ "$scope", "$http", "ingredientsService", "unitsService", function($scope, $http, $ingredientsService, $unitsService) {
    var init, loadIngredients, loadUnits, loadUnitsAndIngredients;
    $scope.ingredients = [];
    $scope.units = [];
    $scope.ingredient = new Ingredient();
    $scope.ingredient.unit = new Unit();
    loadUnits = function() {
        return $http.get("/api/units").success(function(data, status, headers, config) {
            $unitsService.setUnits(data);
            return $scope.units = $unitsService.getUnits();
        });
    };
    loadIngredients = function() {
        return $http.get("/api/ingredients").success(function(data, status, headers, config) {
            $scope.ingredients.splice(0, $scope.ingredients.length);
            $ingredientsService.setIngredients(data);
            return $scope.ingredients = $ingredientsService.getIngredients();
        });
    };
    loadUnitsAndIngredients = function() {
        return $http.get("/api/units").success(function(data, status, headers, config) {
            $unitsService.setUnits(data);
    ...
    init = function() {
        var existingIngredients, existingUnits;
        existingUnits = $unitsService.getUnits();
        existingIngredients = $ingredientsService.getIngredients();
        if (existingUnits.length === 0) {
            if (existingIngredients.length === 0) {
                return loadUnitsAndIngredients();
            } else {
                loadUnits();
                return $scope.ingredients = existingIngredients;
            }
    ...
    $scope.saveIngredient = function() {
        if ($scope.ingredient.name.length > 0 && $scope.ingredient.unit.id) {
            return $http.post("/api/ingredients", {
                ...
                $scope.ingredient = new Ingredient();
                return $scope.setStatusMessage("Інгредієнт успішно збережено.", "success");
            });
        } else {
            return $scope.setStatusMessage("Інгредієнт має мати назву і міру.", "error");
        }
    };
    ...
    $scope.removeIngredient = function(ingredient) {
        return $http["delete"]("/api/ingredients/" + ingredient.id).success(function(data, status, headers, config) {
            var recipe, text, _i, _len, _ref;
            if (data.message === "error") {
            ...
            return $scope.setStatusMessage("Інгредієнт успішно видалено", "success");
        });
    };
    return init();
} ]);

app.controller("RecipesCRUDCtrl", [ "$scope", "$routeParams", "$location", "$rootScope", "$http", "ingredientsService", function($scope, $routeParams, $location, $rootScope, $http, $ingredientsService) {
    var initRecipeForm, setFormModel;
    $scope.ingredients = [];
    ...
    setFormModel = function() {
        if ($routeParams.recipeId) {
            return $http.get("/api/recipes/" + $routeParams.recipeId).success(function(data, status, headers, config) {
                if (data) {
                    return $scope.recipe = new Recipe(data.id, data.name, data.description, "", data.image, data.ingredients);
                }
            });
        } else {
            return $scope.recipe = new Recipe();
        }
    };
    initRecipeForm = function() {
        if ($ingredientsService.getIngredients().length === 0) {
            return $http.get("/api/ingredients").success(function(data, status, headers, config) {
                if (data) {
                    $ingredientsService.setIngredients(data);
                    $scope.ingredients = $ingredientsService.getIngredients();
    ...
    $scope.getRecipeImage = function() {
        if (typeof $scope.recipe.image === "string" && $scope.recipe.image.length > 0) {
    ...
    $scope.chooseIngredient = function(ing) {
        return $scope.ingModel = new Ingredient(ing.id, ing.name, ing.unit);
    };
    $scope.addIngredient = function() {
        if (typeof $scope.ingModel.amount === "string") {
            $scope.ingModel.amount = $scope.ingModel.amount.replace(",", ".");
        }
        if ($scope.ingModel.id > 0 && $scope.ingModel.amount.length > 0) {
            $scope.recipe.ingredients.push($scope.ingModel);
            return $scope.ingModel = new Ingredient();
        }
    };
    $scope.removeIngredient = function(ing) {
        ...
        if (index > -1 && $scope.recipe.ingredients.length > 0) {
            return $scope.recipe.ingredients.splice(index, 1);
        }
    };
        ...
        if ($scope.recipeForm.$invalid === false && $scope.recipe.ingredients.length > 0) {
            if ($scope.recipe.name.length > 0) {
                return $http.post("/api/recipes", {
                    ...
                    ingredients: $scope.recipe.ingredients.map(function(ingr) {
                        return {
                            id: ingr.id,
                            amount: ingr.amount
                        };
                    ...
                    if (data.message === "success" && $scope.recipe.image) {
                        return $http.put("/api/recipes/" + data.recipeId, {
                ...
                return $rootScope.setStatusMessage("Рецепт має містити назву.", "error");
            }
        } else {
            return $rootScope.setStatusMessage("Рецепт має містити принаймні один інгредієнт.", "error");
        }
    };
    return initRecipeForm();
} ]);

app.controller("RecipesCtrl", [ "$scope", "$http", "recipeService", function($scope, $http, $recipeService) {
    var init;
    $scope.recipes = [];
    $scope.removeRecipe = function(recipe) {
        ...
        return $http["delete"](deleteURL).success(function(data, status, headers, config) {
            if (data.message === "error") {
                ...
                $scope.$root.setStatusMessage("Рецепт успішно видалено", "success");
                $scope.$root.$broadcast("recipe:removed", {
                    recipeId: recipe.id
                });
            }
            return recipe.processing = false;
        });
    };
    init = function() {
        ...
        $http.get("/api/recipes/").success(function(data) {
            ...
            return $scope.recipes = $recipeService.getRecipes();
        });
...
app.controller("SummaryCtrl", [ "$scope", "$http", "$filter", "recipeService", function($scope, $http, $filter, $recipeService) {
    var initSummary;
    $scope.weeklySummary = {};
    $scope.showSummaryDate = function() {
        var endDate, startDate, _ref;
        startDate = new Date($scope.weeklySummary.date);
        endDate = new Date(new Date($scope.weeklySummary.date).setDate(((_ref = $scope.weeklySummary.date) != null ? _ref.getDate() : void 0) + 6));
        return "" + startDate.getDate() + "-" + endDate.getDate() + " " + getMonthNameByNumber(startDate.getMonth()) + " " + startDate.getFullYear();
    };
    initSummary = function() {
        var currentWeek, currentYear, todayDate;
        todayDate = new Date();
        currentWeek = +$filter("date")(new Date(todayDate.getFullYear(), todayDate.getMonth(), todayDate.getDate()), "ww", "UTC");
        currentYear = new Date().getFullYear();
        return $http.get("/api/weekly_summary/" + encodeURIComponent(formatDateString(getDateOfISOWeek(currentWeek, currentYear)))).success(function(data) {
            $scope.weeklySummary = data;
            return $scope.weeklySummary.date = new Date($scope.weeklySummary.date);
        });
    };
    return initSummary();
} ]);

app.controller("TopNavCtrl", [ "$scope", "$http", "$location", function($scope, $http, $location) {
    return $scope.signOut = function() {
        return $http.get("/api/signout").success(function(data) {
            if (data.message === "success") {
                $scope.$root.userAuthenticated = false;
                return $location.path("/login");
            }
        }).error(function() {
            return $scope.$root.setStatusMessage("Виникла помилка підчас з'єднаня з сервером. \n Спробуйте іще раз.");
        });
    };
} ]);

app.controller("UnitsCtrl", [ "$scope", "$http", "unitsService", function($scope, $http, $unitsService) {
    var initUnits;
    $scope.units = [];
    $scope.unit = new Unit();
    initUnits = function() {
        if ($unitsService.getUnits().length === 0) {
            return $http.get("/api/units").success(function(data, status, headers, config) {
                $unitsService.setUnits(data);
                return $scope.units = $unitsService.getUnits();
            });
    ...
    $scope.unitActive = function(unit) {
        if (unit.id === $scope.unit.id) {
            return "active";
        } else {
            return "";
        }
    ...
    $scope.saveUnit = function() {
        if ($scope.unit.name.length > 0) {
            return $http.post("/api/units", {
                ...
                $scope.units = $unitsService.getUnits();
                $scope.unit = new Unit();
                return $scope.setStatusMessage("Одиницю виміру успішно збережено.", "success");
            });
        } else {
            return $scope.setStatusMessage("Одиниця міри має мати назву.", "error");
        }
    };
    ...
    $scope.removeUnit = function(unit) {
        return $http["delete"]("/api/units/" + unit.id).success(function(data, status, headers, config) {
            var ingr, text, _i, _len, _ref;
            if (data.message === "error") {
                text = "Не можливо видалити. Одиниця міри використовується у таких інгредієнтах: ";
                _ref = data.ingredients;
                for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                    ingr = _ref[_i];
                    text += "\n" + ingr;
                }
                return $scope.setStatusMessage(text, "error");
            } else {
                $unitsService.setUnits(data);
                $scope.units = $unitsService.getUnits();
                $scope.unit = new Unit();
                return $scope.setStatusMessage("Одиницю міри успішно видалено", "success");
            }
        });
    };
    return initUnits();
...
var DayOfWeek, Ingredient, Recipe, Unit, WeekSummary;
...
Ingredient = function() {
    Ingredient.prototype.unit = new Unit();
    function Ingredient(id, name, unit, amount) {
        this.id = id != null ? id : 0;
        this.name = name != null ? name : "";
        this.amount = amount != null ? amount : "";
        if (unit) {
            this.unit = new Unit(unit.id, unit.name);
        }
    }
    return Ingredient;
}();
...
Recipe = function() {
    Recipe.prototype.processing = false;
    function Recipe(id, name, description, meal, image, ingredients) {
        this.id = id != null ? id : 0;
        this.name = name != null ? name : "";
...
DayOfWeek = function() {
    ...
    DayOfWeek.prototype.today = function() {
        return formatDateString(this.date) === formatDateString(new Date());
    };
    DayOfWeek.prototype.isMealInProgress = function(meal) {
        return this.mealInProgress === meal;
    };
    return DayOfWeek;
}();

WeekSummary = function() {
    function WeekSummary(week, date, ingredients) {
        this.week = week;
        this.date = date;
        this.ingredients = ingredients != null ? ingredients : [];
    }
...
app.service("ingredientsService", [ "$rootScope", "unitsService", function($rootScope, $unitsService) {
    var getById, ingredients, setIngredients;
    ingredients = [];
    setIngredients = function(data) {
        ingredients.splice(0, ingredients.length);
        if (data) {
            return ingredients = data.map(function(ingredient) {
                return new Ingredient(ingredient.id, ingredient.name, ingredient.unit);
            });
        }
    };
    getById = function(id) {
...
app.service("recipeService", [ "$rootScope", "ingredientsService", function($rootScope, $ingredientsService) {
    var getById, recipes, setRecipes;
    recipes = [];
    setRecipes = function(data) {
        if (data) {
            return recipes = data.map(function(recipe) {
                return new Recipe(recipe.id, recipe.name, recipe.description, recipe.meal, recipe.image, recipe.ingredients.map(function(ing) {
                    return new Ingredient(ing.id, ing.name, ing.unit, parseFloat(ing.amount));
                }));
            });
        }
    };
...
app.service("unitsService", [ "$rootScope", function($rootScope) {
    var getById, setUnits, units;
    units = [];
    setUnits = function(data) {
        units.splice(0, units.length);
        if (data) {
            return units = data.map(function(unit) {
...
app.run([ "$rootScope", "$location", "$http", function($rootScope, $location, $http) {
    ...
    $http.get("/api/authentication").success(function(data) {
        $rootScope.userAuthenticated = data.authenticated;
        if (!data.authenticated) {
            $location.path("/login");
        }
        return $rootScope.$on("$routeChangeStart", function(event, next, current) {
            if ($rootScope.userAuthenticated === false) {
                switch (next.templateUrl) {
                  case "views/summary.html":
                  case "views/recipesCRUD.html":
                  case "views/ingredients.html":
                  case "viwes/units.html":
                  case "views/calendar-page.html":
                    return $location.path("/login");
                }
            } else {
    ...
    return $rootScope.recipeDetailsVisible = false;
} ]);
...
app.directive("fileread", function() {
    return function(scope, element, attributes) {
        return element.bind("change", function(event) {
            ...
            img = new Image();
            img.onload = function() {
                var canvas, ctx, height, maxWidthHieght, ratio, width;
                width = img.width;
                height = img.height;
                maxWidthHieght = 167;
                ratio = 0;
                ...
                return scope.$apply(function() {
                    return scope.recipe.image = canvas.toDataURL("image/jpeg");
                });
            };
            reader = new FileReader();
            reader.onload = function(loadEvent) {
                return img.src = loadEvent.target.result;
            };
