    ...
    init = function() {
        return $http.get("/api/units").success(function(data, status, headers, config) {
            $http.get("/api/ingredients").success(function(data, status, headers, config) {
                if (data) {
                    return $scope.ingredients = data.map(function(ingredient) {
                        return new Ingredient(ingredient.id, ingredient.name, ingredient.unit);
                    });
                }
            });
            return $scope.units = data.map(function(unit) {
...
app.controller("UnitsCtrl", [ "$scope", "$http", function($scope, $http) {
    var initUnits, removeUnitOnClient;
    $scope.units = [];
    $scope.unit = new Unit();
    removeUnitOnClient = function(id) {
        var index, unit, _i, _len, _ref;
        _ref = $scope.units;
        ...
        return $scope.units.splice(index, 1);
    };
    initUnits = function() {
        return $http.get("/api/units").success(function(data, status, headers, config) {
            if (data) {
